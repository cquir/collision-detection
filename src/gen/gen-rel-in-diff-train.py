...
def gen(Ntot,split):
    ...
    for dataset,perc in zip([f'train','validation','test'],[0.64, 0.16,0.2]):
        ...
        # rescale inputs
        #numpy.savetxt(f'../../data/datasets/X{label}-mean.dat',numpy.mean(X,axis=0))
        #numpy.savetxt(f'../../data/datasets/X{label}-std.dat',numpy.std(X,axis=0))
